  ...
  class TransactionDetailPayload
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TransactionDetailPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TransactionDetailPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @amount < -2147483648
      ...
      return false if @quantity > 2147483647
      return false if @quantity < 0
      return false if @unit_price.nil?
      return false if @unit_price > 2147483647
      return false if @unit_price < -2147483648
      true
    end
    ...
    def hash
      [amount, description, tax_included_type, tax_rate_type, quantity, unit_price].hash
    end
